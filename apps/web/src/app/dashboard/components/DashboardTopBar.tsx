    ...
    <div className="shrink-0 border-b border-slate-200 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/80 dark:border-slate-800 dark:bg-slate-950/88">
      <div className="flex h-16 items-center justify-between gap-4 px-5 lg:px-6">
        ...
        <div className="hidden min-w-0 flex-1 items-center justify-center gap-2 xl:flex">
          <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-medium text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
            {sourceCount} sources
          </span>
          <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-medium text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
            {activeModeLabel}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={onRefresh}
            className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
          >
            Refresh
          </button>
          <button
      ...
      {(mutationError || mutationNotice || error) && (
        <div className="border-t border-slate-200 px-5 py-2.5 text-[11px] lg:px-6 dark:border-slate-800">
          {mutationError && (
            <span className="inline-flex rounded-full border border-red-200 bg-red-50 px-2.5 py-1 font-medium text-red-700 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-200">
              Error: {mutationError.split(':')[0]}
            </span>
          )}
          {!mutationError && mutationNotice && (
            <span className="inline-flex rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 font-medium text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-200">
              {mutationNotice}
            </span>
          )}
          {!mutationError && !mutationNotice && error && (
