    ...
    <aside className="flex h-full min-h-0 flex-col border-r border-slate-200 bg-slate-50/95 dark:border-slate-800 dark:bg-slate-950/80">
      <div className="shrink-0 border-b border-slate-200 px-5 py-4 dark:border-slate-800">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-200 bg-white text-sm font-semibold text-slate-900 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-50">
            BF
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-[11px]">
          <div className="rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-slate-700 dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-200">
            <div className="font-semibold text-slate-500 dark:text-slate-400">Health</div>
            <div className="mt-1 font-medium">{agentConnected ? 'Connected' : 'Offline'}</div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-slate-700 dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-200">
            <div className="font-semibold text-slate-500 dark:text-slate-400">Ready</div>
            <div className="mt-1 font-medium">{readyCount}</div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-slate-700 dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-200">
            <div className="font-semibold text-slate-500 dark:text-slate-400">Mode</div>
            <div className="mt-1 truncate font-medium">{getActiveContextLabel(activeMode)}</div>
          </div>
        </div>
      </div>
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <button
                ...
                className={`group flex w-full items-center justify-between rounded-2xl border px-3 py-2.5 text-left transition-colors ${
                  isActive
      ...
      <div className="min-h-0 flex-1 px-4 pb-4">
        <div className="flex h-full min-h-0 flex-col rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="shrink-0 border-b border-slate-200 px-4 py-3 dark:border-slate-800">
            ...
            <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
              ...
              <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-slate-600 dark:border-slate-700 dark:bg-slate-950/50 dark:text-slate-300">{writeMode === 'safeWrites' ? 'safe writes' : writeMode}</span>
            </div>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
            {shownSources.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-300">
                No sources yet. Add a local folder in Sources.
              </div>
            ) : (
              <div className="space-y-2">
                {shownSources.map(source => (
                  <div key={source.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 dark:border-slate-800 dark:bg-slate-950/40">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`rounded-full px-2 py-1 text-[10px] font-medium ${getSourceActiveClassName(source.enabled)}`}>
                        {source.enabled ? 'on' : 'off'}
                      </span>
                    </div>
                    <div className="mt-2 flex items-center justify-between gap-2 text-[11px]">
                      <span className={`rounded-full px-2 py-1 font-medium ${getSourceIndexStatusClassName(source.indexStatus)}`}>
                        {getSourceIndexStatusLabel(source)}
                      </span>
                      <span className="rounded-full border border-slate-200 px-2 py-1 font-medium text-slate-500 dark:border-slate-800 dark:text-slate-400">
                        {source.id}
                      </span>
                    </div>
                  </div>
