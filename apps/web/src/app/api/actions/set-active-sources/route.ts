...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const data = await executeAction('/api/set-active-sources', body, auth.bearerToken)
    return NextResponse.json(data)
  } catch (err) {
    if (err instanceof ActionTransportError) return NextResponse.json({ error: err.message }, { status: err.statusCode })
    return NextResponse.json({ error: 'set-active-sources error: ' + String(err) }, { status: 500 })
  }
}
